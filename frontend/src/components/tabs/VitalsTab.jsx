...
import {
  ...
  CheckCircle, XCircle, ArrowRight, Lightbulb, Drop,
...
// For live calculator — age in years → normal ranges
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          ...
          const t = TONE[ok ? "emerald" : c.tone];
          const borderCls = c.ok ? "border-emerald-200 dark:border-emerald-800" : `border-${c.tone}-300 dark:border-${c.tone}-700`;
          return (
            <div key={c.label} className={`rounded-xl border p-3 bg-white dark:bg-slate-900 ${c.ok ? "border-emerald-200 dark:border-emerald-800" : "border-red-200 dark:border-red-800"}`}>
              <div className="font-mono text-[9px] uppercase tracking-widest text-slate-400 mb-1">{c.label}</div>
              <div className={`font-black text-xl leading-none mb-1 ${c.ok ? "text-slate-800 dark:text-white" : "text-red-600 dark:text-red-400"}`}
                   style={{ fontFamily: '"Chivo", system-ui, sans-serif' }}>{c.val}</div>
              <div className="font-mono text-[9px] text-slate-400 mb-1.5">Normal: {c.range}</div>
              <StatusBadge ok={c.ok} />
            </div>
          );
        })}
...
function ReferenceTable() {
  ...
  // Guess the row index from weight
  const suggestedIdx = useMemo(() => {
    if (!weight || !VITALS_ROWS) return null;
    // VITALS_ROWS should have a weight or weightRange field, fall back to index 2
    const idx = VITALS_ROWS.findIndex(r => {
...
function QuickReference() {
  return (
    <div className="space-y-4">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className={`rounded-xl border p-4 ${TONE.amber.border} ${TONE.amber.bg}`}>
          ...
          <div className="space-y-1.5">
            {(TEMP_NOTES || [
              "Fever: ≥ 38.0°C rectal (most accurate), ≥ 37.8°C axillary",
              "Hyperthermia: ≥ 40°C — heat stroke, malignant hyperthermia, NMS",
              "Hypothermia: <36°C — risk of coagulopathy, arrhythmia",
              "Neonates: may NOT mount fever even with serious bacterial infection",
              "Treat fever if child uncomfortable — antipyretics do NOT prevent febrile seizures",
            ]).map((note, i) => (
        ...
        <div className={`rounded-xl border p-4 ${TONE.violet.border} ${TONE.violet.bg}`}>
          ...
          <div className="space-y-1.5">
            {(SPO2_NOTES || [
              "Normal SpO₂: ≥ 94% on room air",
              "Mild hypoxia: 90–93% — supplemental O₂ indicated",
              "Moderate hypoxia: 85–89% — high-flow O₂ or NIV",
              "Severe hypoxia: <85% — intubation likely required",
              "Neonates: pre-ductal SpO₂ (right hand) for true arterial; post-ductal lower in PPHN",
              "Cyanosis clinically visible at SpO₂ ~80–85% — unreliable in anaemia",
            ]).map((note, i) => (
      ...
      {/* PALS thresholds by age — compact table */}
